...
const StorySlide = ({ user, userStories }) => {
  ...
  const goPrev = () => {
    setCurStory((current) =>
      current === 0 ? userStories.length - 1 : current - 1
    );
  };
      ...
      {userStories.length > 1 && (
        <div className="inset-0 w-[400px] h-full absolute flex flex-row justify-between items-center">
          {curStory === 0 ? (
            ...
            <div
              className="z-40 w-[30px] h-auto mx-3 cursor-pointer opacity-70 hover:opacity-100"
              onClick={goPrev}
            >
              {" "}
        ...
        <div className="w-full flex flex-row justify-start items-center">
          <Link to={`/${user.id}`}>
            <div className=" rounded-full w-[35px] h-[35px] relative overflow-hidden flex flex-col justify-center items-center story-active-2">
              <img
                className="w-[30px] h-[30px] absolute rounded-full"
                src={`/${user.id}.jpg`}
              />
            </div>
          </Link>
          <div className="w-[80%] h-[32px] flex flex-col justify-center items-start">
            <div className="w-full h-full flex flex-row justify-start items-center">
              ...
              <div className=" mx-1 text-gray-200">
                {" "}
                • {timeAgo(userStory.date)}
              </div>
            </div>
        ...
        <div className="flex flex-row justify-start items-start absolute inset-0 mx-5">
          {userStories.map((story, idx) => {
            if (idx === curStory) {
              return (
                <div
                  className={`inset-0 mx-1 h-[2px] mt-5 bg-white rounded-full opacity-80 flex flex-col grow relative z-20 ${story.id}`}
                ></div>
              );
            }
            return (
              <div
                className={`inset-0 mx-1 h-[2px] mt-5 bg-white rounded-full opacity-30 flex flex-col grow relative z-20 ${story.id}`}
              ></div>
            );
          })}
        </div>
      ...
      {userStories.map((story, index) => {
        ...
        return (
          <div>
            {thisVideoSrc !== "" && (
              <Slide
                autoPlay={curStory === index ? true : false}
                loop={curStory === index ? true : false}
                muted={curStory === index ? false : true}
                controls
                key={index}
                src={thisVideoSrc}
                state={state}
